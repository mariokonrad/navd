...
{
	struct seatalk_water_temperature_2_t * v;
	...
	if (raw->sentence.command != SEATALK_WATER_TEMPERATURE_2)
		return -2;
	if (raw->sentence.attr.length != 1)
		return -2;

	seatalk->type = raw->sentence.command;
	v = &seatalk->sentence.water_temperature_2;
...
const struct seatalk_sentence_t sentence_27 =
{
	.type = SEATALK_WATER_TEMPERATURE_2,
	.read = read,
	.write = NULL,
	.hton = NULL,
	.ntoh = NULL,
